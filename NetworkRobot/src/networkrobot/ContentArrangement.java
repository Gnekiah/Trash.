 ...
 * 该类提供一个接口：
 * int arrangeContent(String url, String imgDirPath, String textDirPath, int imgID)
 * 函数接受4个参数，其中：
 * url 表示要抓取的目标链接
 * imgDirPath 表示图片保存的目录
 * textDirPath 表示新闻文本保存的目录
 * imgID 表示分配给下一个抓取到的图片的ID
 * 接口返回一个int，表示分配给下一个抓取到的图片的ID
 * @author DouBear
 *
 */
public class ContentArrangement {
     ...
     * @return 返回可分配的下一个图片的ID
     */
    public int arrangeContent(String url, String imgDirPath, String textDirPath, int imgID) {
        this.imgDirPath = imgDirPath;
        // 网页新闻内容的图片ID
        int[] imgIDs = null;
        // 最新的用于分配图片ID的ID，用于记录函数的返回值
        int lastID = imgID;
        ...
        if (doc != null) {
            ...
            // 对没有标题或没有输入保存目录的路径进行处理
            title = title==null ? Long.toString(System.currentTimeMillis()) : title;
            String textAddr = textDirPath==null ? title : textDirPath+title;
            // 获取一个当前目录下唯一的文件名
            textAddr = makeDifferentFilename(textAddr);
            // 创建字节流和字符流，将文本内容写入磁盘
            ...
            try {
                fileWriter = new FileWriter(textAddr);
